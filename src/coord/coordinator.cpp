...
const bool Coordinator::register_agent(const AgentInfo& agent_info)
{
    ...
    {
        log_->print_debug(Logging::Error) << "Failed to register agent " << agent_info.id_ 
    ...
    log_->print_debug(Logging::Message) << "[Coordinator::register_agent] "
...
const bool Coordinator::register_coupling(const CouplingInfo& coupling_info)
{
    ...
    {
        if (!agent_is_known && !neighbor_is_known)
            log_->print_debug(Logging::Warning) << "[Coordinator::register_coupling] Failed to register coupling as agents "
            << coupling_info.agent_id_ << " and " << coupling_info.neighbor_id_ << " are not known." << std::endl;
        else
        {
            log_->print_debug(Logging::Warning) << "[Coordinator::register_coupling] "
                << "Failed to register coupling as agent ";
            if (!agent_is_known)
                log_->print_debug(Logging::Warning) << coupling_info.agent_id_;
            else
                log_->print_debug(Logging::Warning) << coupling_info.neighbor_id_;
            log_->print_debug(Logging::Warning) << " is not known." << std::endl;
        }
        return false;
    }
    ...
    {
        log_->print_debug(Logging::Warning) << "[Coordinator::register_coupling] Failed to register coupling of agent " << coupling_info.agent_id_
    ...
    log_->print_debug(Logging::Message) << "[Coordinator::register_coupling] "
...
const bool Coordinator::deregister_agent(const AgentInfo &agent_info)
{
    ...
    if (agents_.find(agent_info.id_) == agents_.end())
    {
        log_->print_debug(Logging::Warning) << "[Coordinator::deregister_agent] Failed to deregister agent "
...
const bool Coordinator::deregister_coupling(CouplingInfoPtr coupling_info)
{
    ...
    if (!DataConversion::is_element_in_vector(sending_neighbors, *coupling_info))
    {
        log_->print_debug(Logging::Error) << "[Coordinator::deregister_coupling] "
    ...
    log_->print_debug(Logging::Message) << "[Coordinator::deregister_coupling] "
...
void Coordinator::print_network() const
{
    ...
    {
        log_->print_debug(Logging::Message) << "Agent " << id << ": sending neighbors [";
        for (const auto& coupling : sending_neighbors_.find(id)->second)
            log_->print_debug(Logging::Message) << coupling->neighbor_id_ << ", ";

        log_->print_debug(Logging::Message) << "] receiving neighbors [";
        for (const auto& coupling : receiving_neighbors_.find(id)->second)
            log_->print_debug(Logging::Message) << coupling->agent_id_ << ", ";

        log_->print_debug(Logging::Message) << "]" << std::endl;
    }
}
