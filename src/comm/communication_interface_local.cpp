    ...
    log_(log)
{
    ...
    log_->print_debug(Logging::Message) << "[CommunicationInterfaceLocal::CommunicationInterfaceLocal] "
...
void CommunicationInterfaceLocal::handle_disconnect_as_agent(const CommunicationDataPtr& comm_data)
{
    ...
    {
        log_->print_debug(Logging::Message) << "[CommunicationInterfaceLocal::handleDisconnectAsAgent]"
...
void CommunicationInterfaceLocal::close_socket(const CommunicationDataPtr& comm_data) const
{
    ...
    if (ec)
    {
		log_->print_debug(Logging::Error) << "[CommunicationInterfaceLocal::close_socket] "
...
void CommunicationInterfaceLocal::close_shutdown_socket(const CommunicationDataPtr& comm_data) const
{
    ...
    if (ec)
    {
		log_->print_debug(Logging::Error) << "[CommunicationInterfaceLocal::close_shutdown_socket] "
...
void CommunicationInterfaceLocal::handle_disconnect_as_coordinator(const CommunicationDataPtr& comm_data)
{
    ...
    if(comm_data->agent_info_->id_ >= 0)
        ...
        {
			log_->print_debug(Logging::Message) << "[CommunicationInterfaceLocal::handleDisconnectAsCoordinator]"
...
void CommunicationInterfaceLocal::acceptHandler(const std::error_code& ec, CommunicationDataPtr comm_data)
{
    if(ec)
    {
        ...
        log_->print_debug(Logging::Error) << "Failed to accept connection with " << comm_data->communication_info_->agent_type_
...
void CommunicationInterfaceLocal::connectHandler(const std::error_code &ec, CommunicationDataPtr comm_data)
{
    if(ec)
    {
        ...
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::connectHandler] "
            << "Failed to connect with " << comm_data->communication_info_->agent_type_;
        if( comm_data->communication_info_->agent_type_ == "agent" )
            log_->print_debug(Logging::Warning) << " with id " << comm_data->communication_info_->id_;
        log_->print_debug(Logging::Warning) << "." << std::endl << "Error value: " << ec.value() << std::endl
                  << "Error message: " << ec.message() << std::endl;
    ...
    {
        log_->print_debug(Logging::Message) << "[CommunicationInterfaceLocal::connectHandler] "
    ...
    else
    {
        ...
        log_->print_debug(Logging::Message) << "[CommunicationInterfaceLocal::connectHandler] Connected to agent with id "
...
void CommunicationInterfaceLocal::poll_check_connection(const std::error_code &ec, CommunicationDataPtr comm_data)
{
    ...
    if(comm_data->communication_info_->agent_type_ == "coordinator")
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::poll_check_connection] Coordinator is not responding." << std::endl;
    else if(comm_data->communication_info_->agent_type_ == "agent")
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::poll_check_connection] Agent with id " << comm_data->communication_info_->id_
...
const bool CommunicationInterfaceLocal::send_numberOfNeighbors(const int number, const int from, const int to)
{
    ...
    {
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::sendNumberOfNeighbors] "
...
const bool CommunicationInterfaceLocal::send_agentState(const AgentState& state, int from, int to)
{
    ...
    if(comm_data == nullptr)
    {
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::sendAgentState] "
            << "Could not find agent with id " << to << "." << std::endl;
        return false;
    }
...
const bool CommunicationInterfaceLocal::send_desiredAgentState(const AgentState &desired_state, int from, int to)
{
    ...
    if(comm_data == nullptr)
    {
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::sendDesiredState] "
            << "Could not find agent with id " << to << "." << std::endl;
        return false;
    }
...
const std::shared_ptr< AgentState > CommunicationInterfaceLocal::get_agentState_from_agent(int agentId) const
{
    ...
    if (comm_data == nullptr)
    {
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::get_agentStateFromAgent] Agent with id " << agentId
            << " not found." << std::endl;
        return nullptr;
    }
    ...
    if (!comm_data->flag_agentState_for_simulation_)
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::get_agentStateFromAgent] "
...
const std::shared_ptr< AgentState > CommunicationInterfaceLocal::get_desiredAgentState_from_agent(int agentId) const
{
    ...
    if( comm_data == nullptr )
    {
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::get_desiredAgentStateFromAgent] Agent with id "
    ...
    if (!comm_data->flag_desired_agentState_)
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::get_desiredAgentStateFromAgent] Agent with id " << agentId
...
const AgentModelPtr CommunicationInterfaceLocal::get_agentModel(int agentId) const
{
    ...
    if (comm_data == nullptr)
    {
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::get_agentModel] "
	...
	if (!comm_data->flag_agentModel_)
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::get_agentModel] Agent with id " << agentId
...
const std::shared_ptr< std::map<int, CouplingModelPtr> > CommunicationInterfaceLocal::get_couplingModels_from_agent( int agentId ) const
{
    ...
    if (comm_data == nullptr)
    {
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::get_couplingModelsFromAgent] "
            << "Agent with id " << agentId << " not found." << std::endl;
        return nullptr;
    }
...
const bool CommunicationInterfaceLocal::send_couplingState(const CouplingState& state, int from, int to)
{
    ...
    if(comm_data == nullptr)
    {
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::sendCouplingState] Agent with id " << to
            << " not found." << std::endl;
...
const bool CommunicationInterfaceLocal::send_multiplierState(const MultiplierState& multiplier, PenaltyState penalty, int from, int to)
{
    ...
    if(comm_data == nullptr)
    {
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::sendMultiplierState] Agent with id " << to
            << " not found." << std::endl;
...
const bool CommunicationInterfaceLocal::send_convergenceFlag(bool converged, int from)
{
    ...
    if (comm_data == nullptr)
    {
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::sendConvergenceFlag] "
...
const bool CommunicationInterfaceLocal::configure_optimization(const OptimizationInfo& info)
{
    ...
    for (const auto& comm_data : comm_data_to_wait_for)
    {
        if (!comm_data->is_configured)
            log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::configureOptimization] Agent "
...
void CommunicationInterfaceLocal::configureOptimization(const CommunicationDataPtr& comm_data)
{
    ...
    log_->print_debug(Logging::Message) << "[CommunicationInterfaceLocal::configureOptimization] Configured agent "
...
void CommunicationInterfaceLocal::waitFor_connection(const int agents, const int couplings)
{
    ...
    while( (number_of_agents != agents) || (number_of_couplings != couplings) )
    {
        ...
        if (numberOfNotifications_waitForConnection_ != 0)
            log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::waitFor_connection] "
...
const bool CommunicationInterfaceLocal::trigger_step(const ADMMStep& step)
{
    ...
    if (numberOfNotifications_triggerStep_ != 0)
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::triggerStep] "
...
void CommunicationInterfaceLocal::set_simulatedState_for_agent(int agentId, const std::vector<typeRNum>& new_state, typeRNum dt, typeRNum t0)
{
    ...
    if (numberOfNotifications_triggerStep_ != 0)
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::set_simulatedState_for_agent] "
    ...
    std::shared_ptr<std::vector<char>> data_ptr) const
{
    if (ec)
    {
        log_->print_debug(Logging::Error) << "[CommunicationInterfaceLocal::writeHandler] "
...
void CommunicationInterfaceLocal::waitFor_flag_from_coordinator()
{
    ...
    if (comm_data == nullptr)
    { 
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::getFlagFromCoordinator] "
...
const SolutionPtr CommunicationInterfaceLocal::get_solution(unsigned int agent_id) const
{
    ...
    {
        log_->print_debug(Logging::Error) << "[CommunicationInterfaceLocal::get_solution] "
...
const std::vector<SolutionPtr> CommunicationInterfaceLocal::get_solution(const std::string& agents) const
{
    ...
    if (numberOfNotifications_getSolutions_ != 0)
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::getSolutions] "
...
void CommunicationInterfaceLocal::reset_solution(unsigned int agent_id)
{
    if (agent_->get_id() == agent_id)
        ...
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::reset_solution] "
...
void CommunicationInterfaceLocal::reset_solution(const std::string& agents)
{
    if (agents == "all")
        ...
        log_->print_debug(Logging::Error) << "[CommunicationInterfaceLocal::reset_solution] "
...
void CommunicationInterfaceLocal::send_flag_to_agents(const std::string& agents) const
{
    ...
    log_->print_debug(Logging::Error) << "[CommunicationInterfaceLocal::send_flag_to_agents] "
...
void CommunicationInterfaceLocal::send_flag_to_agents(const int agent_id) const
{
    ...
    if (comm_data == nullptr)
    {
        log_->print_debug(Logging::Warning) << "[CommunicationInterfaceLocal::send_flag_to_agents] "
...
{
    log_->print_debug(Logging::Message) << "[CommunicationInterfaceLocal::fromCommunication_send_communicationInfo] "
...
void CommunicationInterfaceLocal::fromCommunication_deregister_agent(const CommunicationDataPtr& comm_data, const AgentInfoPtr& info)
{
    ...
    {
        log_->print_debug(Logging::Error) << "[CommunicationInterfaceLocal::fromCommunication_deregister_agent] "
