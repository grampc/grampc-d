...
const bool Agent::add_neighbor(const dmpc::CouplingInfo &coupling_info, const dmpc::AgentInfo &neighbor_info)
{
...
const bool Agent::add_sendingNeighbor(const dmpc::CouplingInfo &coupling_info, const dmpc::AgentInfo &neighbor_info)
{
    ...
    for(const auto& search_neighbor : neighbors_)
    {
        if( search_neighbor->get_id() == coupling_info.neighbor_id_ )
        {
            ...
            {
                log_->print_debug(Logging::Error) << "[Agent::add_sendingNeighbor] Agent " << get_id() << ": "
...
const bool Agent::add_receivingNeighbor(const dmpc::CouplingInfo &coupling_info, const dmpc::AgentInfo &neighbor_info)
{
    // check if neighbor already exists
    NeighborPtr neighbor = nullptr;
    for( NeighborPtr search_neighbor : neighbors_ )
    {
        if( search_neighbor->get_id() == coupling_info.agent_id_ )
        {
            ...
            {
                log_->print_debug(Logging::Error) << "[Agent::add_receivingNeighbor] Agent " << get_id() << ": "
...
void Agent::remove_neighbor(const dmpc::CouplingInfo &coupling_info)
{
    NeighborPtr neighbor = nullptr;
    ...
    if (neighbor == nullptr)
    {
        log_->print_debug(Logging::Error) << "[Agent::remove_neighbor] Agent " << get_id() << ": "
...
void Agent::set_desiredAgentState(const AgentState& state)
{
    if( compare_stateDimensions( desired_agentState_, state ) )
        ...
        log_->print_debug(Logging::Error) << "[Agent::set_desiredAgentState] Agent " << get_id() << ": "
...
void Agent::set_agentState(const AgentState& state)
{
    ...
    {
        log_->print_debug(Logging::Error) << "[Agent::set_agentState] Agent " << get_id() << ": "
...
void Agent::set_couplingState(const CouplingState& state)
{
    if(compare_stateDimensions( couplingState_, state ))
    {
        ...
        log_->print_debug(Logging::Error) << "[Agent::set_couplingState] Agent " << get_id() << ": "
...
void Agent::set_multiplierState(const MultiplierState& state)
{
    if(compare_stateDimensions( state, multiplierState_ ))
    {
        ...
        log_->print_debug(Logging::Error) << "[Agent::set_multiplierState] Agent " << get_id() << ": "
...
void Agent::set_penaltyState(const PenaltyState &penalty)
{
    if(compare_stateDimensions( penaltyState_, penalty ))
        ...
        log_->print_debug(Logging::Error) << "[Agent::get_penaltyState] Agent " << get_id() << ": "
...
void Agent::fromCommunication_registered_coupling(const CouplingInfo& coupling_info, const AgentInfo& neighbor_info)
{
    ...
    log_->print_debug(Logging::Message) << "[Agent::fromCommunication_registered_coupling] Agent " << get_id() << ": "
...
void Agent::fromCommunication_deregistered_coupling(const CouplingInfo& coupling_info)
{
    ...
    log_->print_debug(Logging::Message) << "[Agent::fromCommunication_deregistered_coupling] Agent " << get_id() << ": "
...
void Agent::fromCommunication_received_agentState(const AgentState& state, int from)
{
    ...
    if(state.i_ == get_id())
    {
        ...
        if( neighbor != nullptr )
            ...
            log_->print_debug(Logging::Warning) << "[Agent::fromCommunication_received_agentState] Agent " << get_id() << ": "
...
void Agent::fromCommunication_received_desiredAgentState(const AgentState& state, int from)
{
    if(state.i_ == from)
    {
        ...
        if( neighbor != nullptr )
            ...
            log_->print_debug(Logging::Warning) << "[Agent::fromCommunication_received_desiredAgentState] Agent " << get_id() << ": "
            << "Could not find neighbor with id " << from << "." << std::endl;
    }
    else
        log_->print_debug(Logging::Warning) << "[Agent::fromCommunication_received_desiredAgentState] Agent " << get_id() << ": "
...
void Agent::fromCommunication_received_couplingState(const CouplingState& coupling, int from)
{
    ...
    if(coupling.i_ == from)
    {
        ...
        if (neighbor == nullptr)
        {
            log_->print_debug(Logging::Warning) << "[Agent::fromCommunication_received_couplingState] Agent " << get_id() << ": "
        ...
        if (compare_stateDimensions(neighbor->get_neighbors_couplingState(), coupling))
            ...
            log_->print_debug(Logging::Error) << "[Agent::fromCommunication_received_couplingState] Agent " << get_id() << ": "
...
void Agent::fromCommunication_received_couplingState(const CouplingState& coupling, const CouplingState& ext_influence_coupling, int from)
{
    ...
    if(coupling.i_ == from)
    {
        ...
        if (compare_stateDimensions(neighbor->get_neighbors_externalInfluence_couplingState(), ext_influence_coupling))
...
void Agent::fromCommunication_received_multiplierState(const MultiplierState& multiplier, PenaltyState penalty, int from)
{
    ...
    if(multiplier.i_ == get_id())
    {
        ...
        if (neighbor == nullptr)
        {
            log_->print_debug(Logging::Warning) << "[Agent::fromCommunication_received_multiplierState] Agent " << get_id() << ": "
                << "Could not find agent with id " << from << "." << std::endl;
            return;
        }
        ...
        if (compare_stateDimensions(neighbor->get_neighbors_coupled_multiplierState(), multiplier))
            ...
            log_->print_debug(Logging::Error) << "[Agent::fromCommunication_received_multiplierState] Agent " << get_id() << ": "
            << "Failed to receive multiplier state, as dimensions don't fit." << std::endl;

        if (compare_stateDimensions(neighbor->get_neighbors_coupled_penaltyState(), penalty))
            neighbor->set_neighbors_coupled_penaltyState(penalty);
        else
            log_->print_debug(Logging::Error) << "[Agent::fromCommunication_received_multiplierState] Agent " << get_id() << ": "
            << "Failed to receive penalty state, as dimensions don't fit." << std::endl;            
    }
    else
    {
		log_->print_debug(Logging::Warning) << "[Agent::fromCommunication_received_multiplierState] Agent " << get_id() << ": "
...
void Agent::fromCommunication_received_numberOfNeighbors(const unsigned int number, const int from)
{
    ...
    if (neighbor == nullptr)
        log_->print_debug(Logging::Warning) << "[Agent::onNumerOfNeighborsReceived] Agent " << get_id() << ": "
...
void Agent::set_neighbors_initial_states()
{
    for (const auto& neighbor : get_neighbors())
    {
        if (optimizationInfo_.APPROX_ApproximateDynamics_)
        {
            ...
            {
                log_->print_debug(Logging::Warning) << "[Agent::set_neighbors_initial_states] Agent " << get_id() << ": "
