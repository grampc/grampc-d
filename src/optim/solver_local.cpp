...
typeRNum SolverLocal::adaptPenaltyParameter( typeRNum primal_residuum, typeRNum dual_residuum, typeRNum penalty )
{
	typeRNum factor;
...
const bool SolverLocal::is_converged() const
{
...
const std::vector<int>& SolverLocal::get_u_index_vji() const
{
    return neighbor_approximation_problem_description_.get_u_index_vji();
}

const std::vector<int>& SolverLocal::get_u_index_xji() const
{
    return default_problem_description_.get_u_index_xji();
}

const std::vector<int>& SolverLocal::get_x_index_xji() const
{
    return neighbor_approximation_problem_description_.get_x_index_xji();
}

const int SolverLocal::get_u_index_uji(int j) const
{
    if(agent_->is_approximatingDynamics())
...
const int SolverLocal::get_u_index_vji(int j) const
{
    if(agent_->is_approximatingDynamics())
        return neighbor_approximation_problem_description_.get_u_index_vji(j);
    else
    {
        log_->print_debug(Logging::Error) << "[SolverLocal::get_u_index_vji]: "
            << "If dynamics are not approximated, there is no external influence." << std::endl;
        return 0;
    }
}

const int SolverLocal::get_u_index_xji(int j) const
{
    return default_problem_description_.get_u_index_xji(j);
}

const int SolverLocal::get_x_index_xji(int j) const
{
    if(agent_->is_approximatingDynamics())
		return neighbor_approximation_problem_description_.get_x_index_xji(j);
	else
	{
		log_->print_debug(Logging::Error) << "[SolverLocal::get_x_index_xji]: "
