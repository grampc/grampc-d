...
void Simulator::distributed_simulation(const std::string& Integrator, typeRNum dt)
{
    ...
    for(const auto& [id, info] : *agents )
    {
        ...
        {
            log_->print_debug(Logging::Warning) << "[Simulator::distributed_simulation] Agent " << id
...
void Simulator::simulate()
{
    ...
    if( Integrator_ == "euler" )
    {
        for(const auto& [id, state] : agentStates_)
        {
            ...
            {
                log_->print_debug(Logging::Warning) << "[Simulator::simulate] Agent " << id
    ...
    else if( Integrator_ == "heun" )
    {
        ...
        log_->print_debug(Logging::Error) << "[Simulator::simulate]: Unknown Integrator." << std::endl;
}
